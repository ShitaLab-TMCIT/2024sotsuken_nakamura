...
% Initialize matrices for overhead and payload times
overhead = zeros(length(Rmin), 1);
payload = zeros(length(Rmin), 1);
payload_t_all = zeros(length(Rmin), 1);
...
for i = 1:length(Rmin)
    ...
    for j = 1:length(distances)
        ...
        % オーバーヘッドの総時間
        total_o = (ACK_t + data_o_t + SIFS + backoff + DIFS + 6) * (D / d1);
    ...
    % Store the overhead and payload times for the current transmission rate
    overhead(i) = ACK_t + data_o_t + SIFS + backoff + DIFS + 6; % オーバーヘッドの総時間
    payload(i) = payload_t; % ペイロードの送信時間

    % Store the total payload time across distances for each transmission rate
    payload_t_all(i) = payload_t; % Save the payload time
end

% 積み上げ棒グラフの作成
bar(TR, [overhead, payload_t_all], 'stacked');

% グラフのラベルとタイトル設定
xlabel('伝送レート [Mbps]');
ylabel('時間 [μs]');
title('伝送レートに対するオーバーヘッドとペイロードの時間');
legend('オーバーヘッド', 'ペイロード', 'Location', 'northeastoutside');
grid on;
hold off;
